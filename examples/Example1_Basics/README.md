...
The PNS problem needs to define its raw materials and products. It is important that this is stored in the PNS problem, not in the materials, as a material is not a raw material or product by nature, only in terms of a given PNS problem.
...
Algorithm SSG  generates solution structures. As with MSG, the constructor expects the problem, and optionally a set of operating units. Algorithm MSG is used Algorithm SSG as a first step, so it is not necessary to call it beforehand.
...
MaterialSet and OperatingUnitSet also provide functionality to find certain items by name or by id. FindByName and FindById return null if the item is not found, while the indexers throw an exception.
...
Parallel production limit (i.e., at most how many operating units can produce a material in any solution) can be set by giving the material node variable, or just the material name.
    ...
    problem.SetMaxParallelProduction(mC, 2);

This does not limit anything in the current case, since there are only two operating units producing material C anyway.

A parallel production limit of 1 has the same effect as a mutual exclusion, but defined with a different meaning, and handled differently in code.

    problem.SetMaxParallelProduction("A", 1);

Don't forget to call FinalizeData() if the problem is modified!